        ...
        while (!back) {
            ...
            try {
                ...
                switch (choice) {
                    ...
                    case "3" -> handleDeleteProduct(FreshProduct.class); // delete fresh product

                    case "4" -> { // update fresh product
                        System.out.println(ansi().bold().fgCyan().a("--- UPDATE FRESH PRODUCT ---").reset());

                        findProductForUpdate(FreshProduct.class).ifPresentOrElse(fresh -> {
                            ...
                            try {
                                Product updated = switch (field) {
                                    case "1" -> BaseFreshProduct.copyOf(fresh)
                                            .withName(readString("New Name: "));
                                    case "2" -> BaseFreshProduct.copyOf(fresh)
                                            .withUnitPrice(readBigDecimal("New Price: "));
                                    case "3" -> BaseFreshProduct.copyOf(fresh)
                                            .withQuantity(readDouble("New Quantity: "));
                                    case "4" -> BaseFreshProduct.copyOf(fresh)
